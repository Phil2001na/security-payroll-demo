...
- Verified live against the Apex Shield demo tenant (never DogForce's real data): the Schedule page's per-cell assignment dialog preemptively disables every incompatible shift option before save, not just after — confirmed both "Night shift the previous day ends too close to this Day shift" (UAT-04, rest/consecutive-shift protection) and "Would exceed the 60-hour weekly cap" (UAT-05, hour-limit protection) fire correctly and block every working-shift type, matching UAT-T04/UAT-T05.
- Found and fixed a real bug while there: the Dashboard's "New here?" onboarding card still described the old 7-step wizard flow (constants → sites → employees → schedule → attendance → payroll) even though the wizard itself was expanded to 13 steps in the PR #2 cloud session yesterday. Updated the blurb to match (`src/routes/_app.dashboard.tsx`).
### 15:45 - autonomous UAT loop, round 2: UAT-08 assessed, UAT-11 completed with a live migration
- UAT-08 (no-candidate/shortage response): confirmed in code and live that the scheduler never auto-assigns an ineligible guard — `buildFillPlan()`'s candidate pool already filters out anyone over the weekly hour cap, short on rest, or in a Night/Day conflict, and tracks per-slot shortfalls (site/date/kind/required/have/short) surfaced via toast instead of silently proceeding. The stricter "name the failed eligibility rules" part of the requirement needs a persistent shortage register, which is UAT-10 — picking that up next in Group B rather than half-building it here.
- UAT-11 (leave cycles) closed out: the schema already tracked each employee's own annual/sick/compassionate cycle window anchored to their `employment_start` date, but never computed the statutory "latest compliant leave date" that `UAT_REQUIREMENTS.md` specifies (Labour Act s.23 — 4 months after cycle end; the 6-month written-agreement extension is deliberately left for UAT-13, which is still decision-gated). Added a generated column `leave_cycles.latest_leave_date` (migration `20260821153000_leave_cycle_latest_compliant_date.sql`, applied live to nakvdkkezgdqxytygtqp, `get_advisors` clean — no new findings) and surfaced it in the Leave page's "Active statutory cycles" table with overdue/due-soon highlighting.
### 16:05 - autonomous UAT loop, round 3: UAT-03 closed out
- Found the bulk of UAT-03 (separate normal-rostered / call-in-relief Sunday categories) already existed: `payroll-engine.ts` already tracks `sunday_hours` (rostered, 1.5×) separately from `sunday_callin_hours` (relief/replacement, full 2× since the guard never agreed to the day) via the existing `schedule_assignments.is_replacement` flag, and the payslip PDF already itemises them as distinct lines ("Sunday (1.5×)" / "Sunday call-in (2×)").
- The one real gap: the Payroll page's summary table collapsed rostered + call-in + public-holiday hours into one combined "Sun/PH" column, which could mask a miscategorised entry from a manager scanning the list — exactly the silent-wrong-category risk the client flagged. Added a compact per-category breakdown line under that cell (`src/routes/_app.payroll.tsx`) whenever more than one category is present. Build verified clean.
- Next: UAT-06 (fair allocation ranking), UAT-10 (shortage register), UAT-12 (leave planner), UAT-16 (premium reporting). UAT-01/02/07/09/13/14/15/17 stay untouched — decision-gated per `uat/2026-08-20/UAT_REQUIREMENTS.md`.

### 16:25 - autonomous UAT loop, round 4: UAT-06 built (ranking only, no hard cap)
- Added a fairness signal to the auto-fill/generate candidate ranking in `src/routes/_app.schedule.tsx`: a new `premium-history` query pulls each guard's Sunday day-shift ("premium") assignments over a trailing 90 days, and `sortPool()` now ranks candidates for a Sunday day slot by fewest recent premium opportunities — inserted right after grade-fit and before home-site/cost, matching the client's "prefer fewer premium opportunities over the familiar guard" wording. Deliberately ranking-only, not a hard exclusion or cap: the exact policy window and whether it's a hard block is UAT-07, still awaiting the client's decision. Build verified clean, no console errors on the Schedule page.
- Next: UAT-10 (shortage register), UAT-12 (leave planner), UAT-16 (premium reporting).

### 16:50 - autonomous UAT loop, round 5: UAT-10 built and end-to-end verified live
- New table `schedule_shortages` (migration `20260821164000_schedule_shortage_register.sql`, applied live to nakvdkkezgdqxytygtqp, `get_advisors` clean) — one row per unfilled slot: site, date, shift kind, required/unmet headcount, `failed_eligibility` (per-employee reason: already rostered elsewhere, over the 60h cap, weekly rest exhausted, or a Night/Day rest conflict), and who attempted the fill. RLS read scope matches `leave_coverage` (admin/operations/supervisor/payroll, security_supervisor limited to their own sites); insert scope matches who can reach the Schedule page.
- `buildFillPlan()` in `src/routes/_app.schedule.tsx` now diagnoses *why* each active-but-unavailable guard was excluded whenever a slot ends up short (mirrors the same checks `buildPool()` already uses, so the reason always matches the real exclusion), and both `autoFillRoster()` and `generateSchedule()` persist the shortfall to the register afterward — best-effort, never blocking the roster fill itself. This completes UAT-08's "state the failed eligibility rules" requirement.
- Added a "Coverage shortages · last 30 days" collapsible report to the Schedule page, grouped by site + shift kind with occurrence counts and the named reasons — the weekly Ops/HR recurring-shortage view UAT-10 asks for.
- Verified end-to-end live: temporarily set a site's requirement to an impossible 999 guards, ran "Generate schedule for range" against the demo tenant, confirmed 5 shortage records were written with 143 named reasons each and the new report card rendered them correctly grouped. Cleaned up afterward — reverted the test requirement and deleted both the fabricated shortage records and the 542 real `schedule_assignments` rows the impossible requirement caused auto-fill to create; confirmed no attendance was logged against any of them before deleting.
- Next: UAT-12 (leave planner), UAT-16 (premium reporting).

### 17:05 - autonomous UAT loop, round 6: UAT-12 built
- Added a "Leave planner — due & overdue for annual leave" card to the Leave page's "Ledger & report" tab (`src/routes/_app.leave.tsx`), above the existing statutory-cycles table. Lists any employee whose `leave_cycles.latest_leave_date` (UAT-11) is within 90 days or already passed, with their current annual balance (reused the existing `balanceFor()` helper rather than re-deriving it), home site, and a simple coverage-risk badge (high/medium/low based on how many other active guards share that home site). Purely informational — it suggests nothing and books nothing; scheduling the leave stays a human decision, matching the requirement.
- Build clean, verified live: the card correctly shows its empty state right now ("No one is due or overdue...") — confirmed via direct SQL that zero of the demo tenant's annual cycles actually fall inside the 90-day window yet, so this is accurate, not a bug.
- Next: UAT-16 (premium/Sunday reporting) — the last Group B item.

### 17:20 - autonomous UAT loop, round 7: UAT-16 built — Group B complete
- Added a "Sunday & premium report" collapsible card to the Payroll page (`src/routes/_app.payroll.tsx`), below the main payroll summary table: employee-level breakdown of rostered-Sunday, Sunday-call-in, and public-holiday hours and pay for the selected period, sorted by total premium pay descending, plus a CSV export. Reused the existing per-employee `calc` fields (`sunday_hours`/`sunday_amount`/`sunday_callin_hours`/`sunday_callin_amount`/`public_holiday_hours`/`public_holiday_amount`) already computed by `payroll-engine.ts` — no new calculation logic, just a management-facing view of numbers the engine already produces. Ties directly to UAT-06's fairness ranking: this is the report a manager checks to see whether that ranking is actually producing an even distribution.
- Build clean, verified live against the demo tenant's real August 2026 pay period — table renders correctly sorted, CSV export button present, no console errors.
- **Group B complete.** All of UAT-03, UAT-06, UAT-10, UAT-12, UAT-16 built and verified. Combined with Group A (UAT-04, UAT-05, UAT-08, UAT-11), 9 of 17 UAT items are now done. The remaining 8 (UAT-01, UAT-02, UAT-07, UAT-09, UAT-13, UAT-14, UAT-15, UAT-17) all stay decision-gated per `uat/2026-08-20/UAT_REQUIREMENTS.md` — untouched, as agreed with Philip.

## 2026-08-20

### 13:30 - UAT requirements extracted for payroll, rostering and leave
- Added `uat/2026-08-20/` with the detailed UAT requirements extraction and a practical UAT checklist/issue register for payroll, roster and leave controls.
- Recorded the decisions needing client/legal confirmation, especially the conflict between the requested midnight pay split and the Labour Act’s majority-of-shift rule.
