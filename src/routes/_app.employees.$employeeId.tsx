...
function EmployeeDetailPage() {
  ...
  const updateDaysPerWeek = useMutation({
    mutationFn: async (days: number) => {
      const { error } = await supabase.from("employees").update({ days_per_week: days }).eq("id", employeeId);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Failed to update working days"),
  });

  const updateGrade = useMutation({
    mutationFn: async (grade: "A+" | "A" | "B" | "C" | "D" | null) => {
      const { error } = await supabase.from("employees").update({ literacy_grade: grade }).eq("id", employeeId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const updateSundays = useMutation({
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Failed to update preference"),
  });

  const updatePreferredShift = useMutation({
    mutationFn: async (shift: "day" | "night" | "both") => {
      const { error } = await supabase.from("employees").update({ preferred_shift: shift }).eq("id", employeeId);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    onError: (err) => toast.error(err instanceof Error ? err.message : "Failed to update preferred shift"),
  });
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <CardContent className="space-y-2 text-sm">
            ...
            <div className="flex items-center justify-between gap-4 py-1">
              ...
              <Select
                ...
                onValueChange={(v) => updateGrade.mutate(v === "none" ? null : v as "A+" | "A" | "B" | "C" | "D")}
        ...
        {!isManagement && (
          <Card>
            <CardHeader><CardTitle className="text-base">Leave</CardTitle></CardHeader>
            <CardContent className="space-y-2 text-sm">
              <Row label="Annual leave balance" value={`${Number(leave?.annual_days ?? 0).toFixed(2)} days`} mono />
              <Row label="Accrual rate" value="1 day / 12 worked" mono />
              <Row label="Sick days" value={`${Number(leave?.sick_days ?? 0).toFixed(2)}`} mono />
              <Row label="Compassionate" value={`${Number(leave?.compassionate_days ?? 0).toFixed(2)}`} mono />
